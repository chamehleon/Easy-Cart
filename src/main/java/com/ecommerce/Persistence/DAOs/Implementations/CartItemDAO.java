...
public class CartItemDAO extends GenericDAOImpl<CartItem> {
    ...
    public List<ProductWithQuantityDto> retrieveCartItemNotExistInLocalStorageRepo(int cartId, Integer[] productIdsArray, EntityManager entityManager) {
    ...
    public List<ProductWithQuantityDto> retrieveProductWithQuantityDtoRepo(int cartId, EntityManager entityManager) {
        // Constructing the JPQL query
        String jpql = "SELECT ci FROM CartItem ci " +
                "WHERE ci.cart.id = :cartId";
        // Creating the query
